...
import DateInputExtension from '../Extensions/DateInputExtension';
...
class Calendar {
  ...
  private dateInput: DateInputExtension;
  ...
  constructor(node: any, options: object = {}) {
    ...
    this.dateInput = new DateInputExtension(this);
    this.updateRoot();
  ...
  private updateEventListeners(): void {
    ...
    this.root.querySelectorAll('input').forEach(input => {
      input.removeEventListener('focus', this.dateInput.handleFocus);
      input.addEventListener('focus', this.dateInput.handleFocus);
    })

    this.root.querySelectorAll('input').forEach(input => {
      input.removeEventListener('input', this.dateInput.handleInput);
      input.addEventListener('input', this.dateInput.handleInput);
    })
  }
